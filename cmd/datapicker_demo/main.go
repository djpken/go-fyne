...
func main() {
	...
	dateInput.ActionItem = widget.NewButtonWithIcon("", theme.MoreHorizontalIcon(), func() {
		...
		datepicker := component.NewDatePicker(when, time.Monday, func(when time.Time, ok bool) {
			if ok {
	...
	datetimeInput.ActionItem = widget.NewButtonWithIcon("", theme.MoreHorizontalIcon(), func() {
		...
		picker := component.NewDateTimePicker(when, time.Monday, func(when time.Time, ok bool) {
			if ok {
