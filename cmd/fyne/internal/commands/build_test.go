	...
	expected := []mockRunner{
		{
			...
			mockReturn: mockReturn{
				ret: []byte("{ \"Module\": { \"Path\": \"github.com/djpken/go-fyne\"} }"),
			},
		},
		{
...
func Test_FyneGoMod(t *testing.T) {
	jsonTests := []jsonTest{
		{false, []byte(`{"Module": {"Path": "github.com/fyne-io/calculator"},"Go": "1.14",	"Require": [ { "Path": "github.com/djpken/go-fyne","Version": "v2.1.4"} ] }`)},
		{true, []byte(`{ "Module": {"Path": "github.com/djpken/go-fyne"},"Require": [{ "Path": "test","Version": "v2.1.4"} ] }`)},
		{true, []byte(`{"Module": {"Path": "github.com/fyne-io/calculator"},"Go": "1.14",	"Require": [ { "Path": "github.com/djpken/go-fyne","Version": "v2.2.0"} ] }`)},
	}
